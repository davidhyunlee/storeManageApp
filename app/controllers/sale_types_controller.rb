class SaleTypesController < ApplicationController
  ...
  # GET /sale_types.json
  def index
    @sale_types = SaleType.all.page(params[:page])
    authorize @sale_types
  end
  ...
  def create
    ...
    respond_to do |format|
      if @sale_type.save
        format.html { redirect_to sale_types_path, notice: 'Sale type was successfully created.' }
  ...
  private
    ...
    def sale_type_params
      params.require(:sale_type).permit(:name, :description, :tracked)
    end
end
